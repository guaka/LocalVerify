    ...
    var body: some View {
        List {
            Section {
                ...
                Text("A sysdiagnose is an archive of iPhone diagnostic records. iOS creates it; LocalVerify analyzes the file you choose. Allow several minutes and some free storage.")
            ...
            Section("1 · Generate the archive") {
                ...
                Text("Wait several minutes for collection to finish. LocalVerify cannot start this collection automatically.")
            }
            Section("Alternative · Use AssistiveTouch") {
                ...
                if #available(iOS 26.0, *) {
                    ...
                    Text("Opens Settings only. If iOS stops at Accessibility, continue to Touch → AssistiveTouch. You choose whether to enable it; LocalVerify cannot change this setting.").font(.footnote)
                } else {
                ...
                Text("This is a one-time diagnostic collection action, not continuous monitoring. It is separate from Share iPhone Analytics, which shares analytics with Apple. You do not need to enable that sharing setting for LocalVerify.").font(.footnote)
            }
            Section("2 · Find it in Settings") {
                Text("Open the iPhone Settings app and follow:")
                Text("Privacy & Security\n→ Analytics & Improvements\n→ Analytics Data").font(.headline)
                Text("iOS does not provide a supported direct shortcut from LocalVerify to this screen. Open Settings manually and follow the path above.").font(.footnote)
            ...
            Section("4 · Import into LocalVerify") {
                Text("Return to LocalVerify, then tap Import sysdiagnose.")
                Text("In the file picker, open Browse → On My iPhone → LocalVerify → Imports and select the .tar.gz file you saved. Analysis starts after the copy completes.")
            ...
            Section("If you cannot find the file") {
                ...
                Text("If the LocalVerify folder is missing in Files, open LocalVerify once, return to Files → Browse, and reopen On My iPhone.")
            ...
            Section("After importing") {
                Text("LocalVerify keeps its own protected case copy. The file you saved in Files remains separate; you can remove it there when you no longer need it. Deleting a LocalVerify case does not delete the original in Settings or Files.")
                Text("Analysis runs locally. Nothing is uploaded by LocalVerify. Findings are leads for review, not proof that a device is clean or compromised.")
            }
            ...
            .alert("Could not open AssistiveTouch", isPresented: $settingsError) {
                ...
                Text("Open Settings → Accessibility → Touch → AssistiveTouch manually. No settings have been changed by LocalVerify.")
            }
    }
}
