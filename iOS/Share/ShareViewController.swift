...
final class ShareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        label.text = "Import this diagnostic archive? It will be stored locally until you analyze or delete it in LocalVerify."
        let button = UIButton(type: .system); button.setTitle("Save to LocalVerify", for: .normal); button.addTarget(self, action: #selector(save), for: .touchUpInside)
    ...
    @objc private func save() {
        ...
        provider.loadFileRepresentation(forTypeIdentifier: type) { [weak self] url, error in
            do {
                ...
                DispatchQueue.main.async { self?.label.text = "Saved. Open LocalVerify to analyze." }
